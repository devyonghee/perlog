...
const openNewForm = (open, newType = '') => (dispatch, getState) => {
    ...
    const server = serverState.list.find(server => server.url === file.url);
    ...
    file.token ? dispatch(serverActions.connectByToken(file.name, file.url, file.token)) : dispatch(serverActions.connect(file.url, file.name));
};
